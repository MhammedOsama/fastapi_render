...
lstm_model_path = os.path.join(MODELS_DIR, "lstm_without_masking.h5")
audio_model_path = os.path.join(MODELS_DIR, "audio_model.h5")

if os.path.exists(lstm_model_path):
    saved_model = load_model(lstm_model_path)
else:
    print("⚠️ Warning: Model lstm_without_masking.h5 not found!")

if os.path.exists(audio_model_path):
    model = load_model(audio_model_path)
else:
    print("⚠️ Warning: Model audio_model.h5 not found!")
...
def final_decision(input_video):
    ...
    # If replicated voice predictions are shorter than facial predictions, pad them
        ...
        replicated_voice_predictions = np.pad(replicated_voice_predictions,
